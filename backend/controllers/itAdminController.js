...
function maskPhone(phone) {
  ...
  return (
    digits.substring(0, 4) + "******" + digits.substring(digits.length - 2)
  );
}
...
exports.createAdminUser = async (req, res) => {
  try {
    ...
    const {
      ...
      subDepartmentMap: SUB_DEPARTMENT_MAP,
    ...
    const insertAttempts = [
      ...
      {
        // legacy last_name
        sql: "INSERT INTO admin_users (username, password, role, department, sub_department, first_name, last_name, other_names, email, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)",
        params: [
          username,
          hashedPassword,
          role,
      ...
      },
      {
        ...
        sql: "INSERT INTO admin_users (username, password, role, sub_department, first_name, last_login, created_at, updated_at, last_name, is_active) VALUES (?, ?, ?, ?, ?, NULL, NOW(), NOW(), ?, TRUE)",
        params: [
          username,
          hashedPassword,
          role,
    ...
    for (let i = 0; i < insertAttempts.length; i++) {
      ...
      try {
        ...
        if (
          !(err && err.code === "ER_BAD_FIELD_ERROR") ||
          i === insertAttempts.length - 1
        ) {
          if (!inserted) {
            console.error(
              "Create admin user insert failed (attempt",
              i,
              "):",
              err.message
            );
...
exports.getUserCreationAudit = async (req, res) => {
  try {
    ...
    const params = [];
    if (adminId) {
      conditions.push("uca.created_by_admin_id = ?");
      params.push(adminId);
    }
    if (userId) {
      conditions.push("uca.user_id = ?");
      params.push(userId);
    }
    if (nationalId) {
      conditions.push("uca.user_national_id = ?");
      params.push(nationalId);
    }
    if (department) {
      conditions.push("uca.user_department = ?");
      params.push(department);
    }
    if (employmentNature) {
      conditions.push("uca.user_employment_nature = ?");
      params.push(employmentNature);
    }
    if (sub_department) {
      conditions.push("uca.user_sub_department = ?");
      params.push(sub_department);
    }
    if (from) {
      conditions.push("uca.created_at >= ?");
    ...
    if (search && search.trim()) {
      ...
      conditions.push(
        `(LOWER(uca.user_email) LIKE ? OR LOWER(uca.user_national_id) LIKE ? OR LOWER(uca.user_department) LIKE ? OR LOWER(uca.user_sub_department) LIKE ?)`
      );
    ...
    const [countRows] = await pool.query(
      `SELECT COUNT(*) as total FROM user_creation_audit uca ${whereClause}`,
      params
    );
    ...
    const [rows] = await pool.query(
      `SELECT id, user_id, created_by_admin_id, admin_role, ip_address, user_national_id, user_email, user_department, user_sub_department, user_employment_nature, created_at
       ...
       ORDER BY ${
         orderColumn === "user_id"
           ? "uca.user_id"
           : orderColumn === "created_by_admin_id"
           ? "uca.created_by_admin_id"
           : "uca." + orderColumn
       } ${direction}, uca.id DESC
       LIMIT ? OFFSET ?`,
...
exports.getAdminCreationAudit = async (req, res) => {
  try {
    ...
    const params = [];
    if (createdByAdminId) {
      conditions.push("aca.created_by_admin_id = ?");
      params.push(createdByAdminId);
    }
    if (adminId) {
      conditions.push("aca.admin_id = ?");
      params.push(adminId);
    }
    if (newRole) {
      conditions.push("aca.new_admin_role = ?");
      params.push(newRole);
    }
    if (department) {
      conditions.push("aca.new_admin_department = ?");
      params.push(department);
    }
    if (sub_department) {
      conditions.push("aca.new_admin_sub_department = ?");
      params.push(sub_department);
    }
    if (from) {
      conditions.push("aca.created_at >= ?");
      params.push(from + " 00:00:00");
    }
    if (to) {
      conditions.push("aca.created_at <= ?");
    ...
    if (search && search.trim()) {
      const term = "%" + search.toLowerCase().trim() + "%";
      conditions.push(
        "(LOWER(aca.new_admin_username) LIKE ? OR LOWER(aca.new_admin_role) LIKE ? OR LOWER(aca.new_admin_department) LIKE ? OR LOWER(aca.new_admin_sub_department) LIKE ? OR LOWER(aca.new_admin_first_name) LIKE ? OR LOWER(aca.new_admin_surname) LIKE ?)"
      );
    ...
    const sortable = new Set([
      ...
      "new_admin_sub_department",
      "new_admin_username",
    ]);
    const column = sortable.has(sortBy) ? sortBy : "created_at";
    const direction = String(sortDir).toLowerCase() === "asc" ? "ASC" : "DESC";
    const [countRows] = await pool.query(
      `SELECT COUNT(*) as total FROM admin_creation_audit aca ${whereClause}`,
      params
    );
    const total = countRows[0]?.total || 0;
    const [rows] = await pool.query(
...
exports.exportAdminCreationAuditCsv = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="admin_creation_audit.csv"'
    );
    return res.send(csv);
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Server error exporting CSV" });
  }
};
...
exports.exportAdminCreationAuditPdf = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="admin_creation_audit.pdf"'
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Server error exporting PDF" });
  }
};
...
exports.exportUserCreationAuditCsv = async (req, res) => {
  ...
  SELECT id, user_id, created_by_admin_id, admin_role, ip_address, user_national_id, user_email, user_department, user_sub_department, user_employment_nature, created_at
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="user_creation_audit.csv"'
    );
    return res.send(csv);
  } catch (error) {
...
exports.exportUserCreationAuditPdf = async (req, res) => {
  ...
  SELECT id, user_id, created_by_admin_id, admin_role, ip_address, user_national_id, user_email, user_department, user_sub_department, user_employment_nature, created_at
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="user_creation_audit.pdf"'
    );
    const doc = new PDFDocument({ margin: 40, size: "A4" });
    doc.pipe(res);
...
exports.revealUserOtp = async (req, res) => {
  try {
    ...
    if (!["it_admin", "super_admin", "it", "super"].includes(role)) {
      return res
        .status(403)
        .json({ success: false, message: "Insufficient role to reveal OTP." });
    }
    ...
    const [rows] = await pool.query(
      `SELECT id, department, sub_department, otp_code, otp_expires_at, password_changed, phone_number, otp_request_count, otp_request_window_start FROM users WHERE id = ?`,
      [userId]
    );
    if (!rows.length)
      return res
    ...
    let windowStart = user.otp_request_window_start
      ? new Date(user.otp_request_window_start)
      : null;
    ...
    if (regenerate || !hasActive) {
      ...
      if (wantGenerate) {
        ...
        if (windowExpired) {
          await pool.query(
            `UPDATE users SET otp_code = ?, otp_expires_at = ?, otp_request_count = ?, otp_request_window_start = NOW() WHERE id = ?`,
            [activeCode, expiry, otp_request_count, userId]
          );
        } else {
          await pool.query(
            `UPDATE users SET otp_code = ?, otp_expires_at = ?, otp_request_count = ? WHERE id = ?`,
            [activeCode, expiry, otp_request_count, userId]
          );
        }
        ...
        if (user.phone_number) {
          try {
            ...
            await sendSMS({
              to: user.phone_number,
              body: `Your WDP one-time code is ${activeCode}. It expires in 6 hours.`,
            });
            smsSent = true;
          } catch (e) {
            console.warn(
              "Failed to send OTP SMS (IT reveal regenerate):",
              e.message
            );
          }
        }
    ...
    try {
      ...
      const ADMIN_THRESHOLD = parseInt(
        process.env.OTP_DISCLOSURE_ADMIN_THRESHOLD || "10",
        10
      ); // default 10
      const GLOBAL_THRESHOLD = parseInt(
        process.env.OTP_DISCLOSURE_GLOBAL_THRESHOLD || "100",
        10
      ); // default 100
      const [recentAdmin] = await pool.query(
        `SELECT COUNT(*) AS c FROM otp_disclosure_audit WHERE admin_id = ? AND created_at >= (NOW() - INTERVAL 1 HOUR)`,
        [adminCtx.adminId || 0]
      );
      const [recentGlobal] = await pool.query(
        `SELECT COUNT(*) AS c FROM otp_disclosure_audit WHERE created_at >= (NOW() - INTERVAL 1 HOUR)`
      );
      if (
        (recentAdmin[0]?.c || 0) > ADMIN_THRESHOLD ||
        (recentGlobal[0]?.c || 0) > GLOBAL_THRESHOLD
      ) {
        alertNeeded = true;
      }
    } catch (auditErr) {
    ...
    if (alertNeeded) {
      ...
      try {
        ...
        await sendEmail({
          ...
          html: `<p><strong>Alert:</strong> Potential unusual OTP disclosure activity.</p><p>Admin: ${
            adminCtx.adminId
          }<br/>Role: ${
            adminCtx.role
          }<br/>Time: ${new Date().toISOString()}</p><p>Review the <code>otp_disclosure_audit</code> table for details.</p>`,
        });
      } catch (e) {
...
exports.getOtpDisclosureAudit = async (req, res) => {
  try {
    ...
    const params = [];
    if (adminId) {
      conditions.push("oda.admin_id = ?");
      params.push(adminId);
    }
    if (userId) {
      conditions.push("oda.user_id = ?");
      params.push(userId);
    }
    if (action) {
      conditions.push("oda.action = ?");
      params.push(action);
    }
    if (from) {
      conditions.push("oda.created_at >= ?");
      params.push(from + " 00:00:00");
    }
    if (to) {
      conditions.push("oda.created_at <= ?");
    ...
    if (search) {
      const term = "%" + search.toLowerCase().trim() + "%";
      conditions.push(
        "(LOWER(oda.reason) LIKE ? OR LOWER(oda.admin_role) LIKE ? OR LOWER(oda.admin_department) LIKE ? OR LOWER(oda.admin_sub_department) LIKE ?)"
      );
      params.push(term, term, term, term);
    }
    ...
    if (role !== "super_admin" && role !== "super") {
      if (adminCtx.department) {
        conditions.push(
          "(oda.admin_department = ? OR oda.admin_department IS NULL)"
        );
    ...
    const whereClause = conditions.length
      ...
      : "";
    const [countRows] = await pool.query(
      `SELECT COUNT(*) as total FROM otp_disclosure_audit oda ${whereClause}`,
      params
    );
    const total = countRows[0]?.total || 0;
    const [rows] = await pool.query(
