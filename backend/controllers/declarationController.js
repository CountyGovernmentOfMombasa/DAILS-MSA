...
exports.downloadDeclarationPDF = async (req, res) => {
  try {
    ...
    // Generate unencrypted PDF buffer first
    const {
      buffer: unencryptedBuffer,
      base,
      password,
      passwordInstruction,
    } = await generateDeclarationPDF(declarationId, { skipEncryption: true });

    let finalBuffer = unencryptedBuffer;
    ...
    if (password) {
      try {
        ...
        const inputStream = new ReadableStream(unencryptedBuffer);
        const outputStream = new WritableStream();

        const permissions = {
          userPassword: password,
          ownerPassword: process.env.PDF_OWNER_PASSWORD || password,
          printing: process.env.PDF_PERMIT_PRINTING || "high",
          modifying: process.env.PDF_ALLOW_MODIFY === "true",
          copying: process.env.PDF_ALLOW_COPY === "true",
          annotating: process.env.PDF_ALLOW_ANNOTATE === "true",
          fillingForms: process.env.PDF_ALLOW_FILL_FORMS === "true",
          contentAccessibility: process.env.PDF_ALLOW_CONTENT_ACCESS === "true",
          documentAssembly: process.env.PDF_ALLOW_DOC_ASSEMBLY === "true",
        };

        muhammara.recrypt(inputStream, outputStream, permissions);
        finalBuffer = outputStream.toBuffer();
        encryptionApplied = true;
      } catch (e) {
        console.error(
          "PDF encryption with muhammara failed. Serving unencrypted file. Error:",
          e.message
        );
      }
    }
...
exports.submitDeclaration = async (req, res) => {
  try {
    ...
    try {
      ...
      const sendSMS = require("../util/sendSMS");
      const { generateDeclarationPDF } = require("../util/pdfBuilder");
      // Generate unencrypted PDF, then encrypt it with muhammara
      const {
        buffer: unencryptedBuffer,
        base,
        password,
      ...
      let finalPdfBuffer = unencryptedBuffer;
      if (password) {
        try {
          const muhammara = require("muhammara");
          const { WritableStream, ReadableStream } = require("memory-streams");
          ...
          const permissions = {
            userPassword: password,
            ownerPassword: process.env.PDF_OWNER_PASSWORD || password,
            printing: "high", // Default for email attachments
          };
          muhammara.recrypt(inputStream, outputStream, permissions);
          finalPdfBuffer = outputStream.toBuffer();
        } catch (encErr) {
          console.error(
            "PDF encryption for email failed. Attaching unencrypted file. Error:",
            encErr.message
          );
          // Fallback to sending the unencrypted buffer
          finalPdfBuffer = unencryptedBuffer;
        }
      }
      ...
      await sendEmail({
        ...
        attachments: [
          { filename, content: finalPdfBuffer, contentType: "application/pdf" },
        ],
      });
