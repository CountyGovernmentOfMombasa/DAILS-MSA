...
async function alreadyApplied(filename, checksum) {
  const [rows] = await pool.query(
    "SELECT 1 FROM schema_migrations WHERE filename = ? AND checksum = ? LIMIT 1",
    [filename, checksum]
  );
...
async function runMigrationFile(fullPath, filename) {
  ...
  const hasExplicitTx =
    /\bSTART\s+TRANSACTION\b/i.test(sql) && /\bCOMMIT\b/i.test(sql);
  const statements = hasExplicitTx
    ? [sql]
    : sql.split(/;\s*\n/).filter((s) => s.trim());
  ...
  try {
    ...
    if (isBenign) {
      ...
      try {
        // Use INSERT IGNORE or a catch block for the recording itself
        await pool.query(
          "INSERT IGNORE INTO schema_migrations (filename, checksum) VALUES (?, ?)",
          [filename, checksum]
        );
      } catch (_) {
        /* Final safety net, should not be hit with INSERT IGNORE */
      }
    } else {
