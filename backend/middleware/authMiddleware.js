...
const parseMinutes = (v, d) => {
  const n = parseInt(v, 10);
  return isNaN(n) ? d : n;
};
const INACTIVITY_MINUTES = parseMinutes(
...
exports.verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    const isResetToken = !!(decoded && decoded.reset === true);
    ...
    // Only perform inactivity check for standard user session tokens.
    if (decoded && decoded.id && !isOtpToken && !isResetToken) {
      try {
    ...
    try {
      ...
      if (fullUser) {
        // Merge full user profile, but explicitly preserve special token claims.
        // This is more robust than relying on spread order.
        req.user = { ...fullUser, ...decoded };
        if (decoded.otp) req.user.otp = true;
        if (decoded.reset) req.user.reset = true;
      }
    } catch (hydrateErr) {
