...
function enforceFinancialTypes(req, res, next) {
  ...
  details.push(
    ...validateFinancialArray(rootIncome, allowedIncome, "biennial_income")
  );
  details.push(...validateFinancialArray(rootAssets, allowedAssets, "assets"));
  details.push(
    ...validateFinancialArray(
      rootLiabilities,
      allowedLiabilities,
      "liabilities"
    )
  ...
  spouses.forEach((s, si) => {
    details.push(
      ...validateFinancialArray(
        normalizeArr(s?.biennial_income),
        allowedIncome,
        `spouses[${si}].biennial_income`
      )
    );
    details.push(
      ...validateFinancialArray(
        normalizeArr(s?.assets),
        allowedAssets,
        `spouses[${si}].assets`
      )
    );
    details.push(
      ...validateFinancialArray(
        normalizeArr(s?.liabilities),
        allowedLiabilities,
        `spouses[${si}].liabilities`
      )
    );
  });
  ...
  children.forEach((c, ci) => {
    details.push(
      ...validateFinancialArray(
        normalizeArr(c?.biennial_income),
        allowedIncome,
        `children[${ci}].biennial_income`
      )
    );
    details.push(
      ...validateFinancialArray(
        normalizeArr(c?.assets),
        allowedAssets,
        `children[${ci}].assets`
      )
    );
    details.push(
      ...validateFinancialArray(
        normalizeArr(c?.liabilities),
        allowedLiabilities,
        `children[${ci}].liabilities`
      )
    );
  });
  ...
  if (details.length) {
    return res.status(400).json({
      success: false,
      message: "Invalid financial types detected",
      code: "VALIDATION_FINANCIAL_TYPES",
      details,
    });
  }
  return next();
