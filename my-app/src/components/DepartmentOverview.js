...
function mapToCanonical(raw, canonicalIndex) {
  ...
  // Fallback: partial matching (contains or starts/ends with)
 ...
 */
const DepartmentOverview = ({ declarations = [], employeeTotalsByDepartment = null, showUnknown = true, backendStats = null, loading = false, onRefresh = null }) => {
  const { departments: dynamicDeps } = useDepartments();
  const CANONICAL_DEPARTMENTS = dynamicDeps && dynamicDeps.length ? dynamicDeps : require('../constants/departments').DEPARTMENTS;
  const { rows, overallTotalsSource, hadUnknown, backendUsed } = useMemo(() => {
    ...
    if (backendStats && backendStats.counts) {
      const totalUnique = backendStats.totalUniqueEmployeesWithDeclarations || Object.values(backendStats.counts).reduce((a, v) => a + (typeof v === 'number' ? v : 0), 0) + (backendStats.unknown || 0);
      const rows = CANONICAL_DEPARTMENTS.map(canon => {
        const declared = backendStats.counts[canon] || 0;
        const totalForDept = employeeTotalsByDepartment ? (employeeTotalsByDepartment[canon] ?? null) : declared; // if separate totals passed, use them
        const percent = totalUnique > 0 ? ((declared / totalUnique) * 100).toFixed(1) : '0.0';
      ...
      if (showUnknown && backendStats.unknown > 0) {
        ...
        const totalForUnknown = employeeTotalsByDepartment ? (employeeTotalsByDepartment['Unknown / Other'] ?? null) : unknownDeclared;
        const percent = totalUnique > 0 ? ((unknownDeclared / totalUnique) * 100).toFixed(1) : '0.0';
        rows.push({ department: 'Unknown / Other', declared: unknownDeclared, total: totalForUnknown, percent });
      }
      return { rows, overallTotalsSource: 'backend (unique declarants)', hadUnknown: (backendStats.unknown || 0) > 0, backendUsed: true };
    }
    ...
    const parseDatePriority = (d) => {
      ...
      const ts = dateStr ? Date.parse(dateStr.replace(/(\d{2})\/(\d{2})\/(\d{4})/, '$3-$2-$1')) : undefined; // handle DD/MM/YYYY
    ...
    for (const d of declarations) {
      ...
      if (!employeeMap.has(key)) {
        employeeMap.set(key, { dept: canonical, dateValue: dateVal, isKnown: !!canonical });
      } else {
        ...
        if (dateVal >= prev.dateValue) {
          if (!prev.isKnown && canonical) {
            employeeMap.set(key, { dept: canonical, dateValue: dateVal, isKnown: true });
          } else if (canonical && prev.isKnown) {
            employeeMap.set(key, { dept: canonical, dateValue: dateVal, isKnown: true });
          } else if (!prev.isKnown && !canonical) {
            employeeMap.set(key, { dept: null, dateValue: dateVal, isKnown: false });
          }
        }
      }
    }
    ...
    for (const canon of CANONICAL_DEPARTMENTS) {
      ...
      const totalForDept = employeeTotalsByDepartment ? (employeeTotalsByDepartment[canon] ?? null) : declared;
      let percent;
      if (employeeTotalsByDepartment) {
        percent = totalForDept > 0 ? ((declared / totalForDept) * 100).toFixed(1) : '0.0';
      } else {
        // Still base percentage on overall unique declarants so distribution sums ~100%
        percent = overallUnique > 0 ? ((declared / overallUnique) * 100).toFixed(1) : '0.0';
      }
      rows.push({ department: canon, declared, total: totalForDept, percent });
    }

    if (showUnknown && unknownCount > 0) {
      const totalForUnknown = employeeTotalsByDepartment ? employeeTotalsByDepartment['Unknown / Other'] ?? null : overallUnique;
      let percent;
      if (employeeTotalsByDepartment) {
        percent = totalForUnknown > 0 ? ((unknownCount / totalForUnknown) * 100).toFixed(1) : '0.0';
      } else {
        percent = overallUnique > 0 ? ((unknownCount / overallUnique) * 100).toFixed(1) : '0.0';
      }
      rows.push({ department: 'Unknown / Other', declared: unknownCount, total: totalForUnknown, percent });
    }

    return { rows, overallTotalsSource: employeeTotalsByDepartment ? 'department totals' : 'unique declarants', hadUnknown: unknownCount > 0, backendUsed: false };
  }, [declarations, employeeTotalsByDepartment, showUnknown, backendStats, CANONICAL_DEPARTMENTS]);
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
        ...
        <div className="d-flex flex-column align-items-end">
          <small className="text-light fst-italic mb-1">
            Percent base: {overallTotalsSource}{hadUnknown ? ' (includes unknown bucket)' : ''}{backendUsed ? ' ✔' : ''}
          </small>
          {onRefresh && (
            <button className="btn btn-sm btn-outline-light" onClick={onRefresh} disabled={loading}>
      ...
      <div className="card-body">
        {loading && (
          <div className="alert alert-info py-2">Loading department statistics...</div>
        )}
          ...
          <table className="table table-striped table-hover">
            <thead className="table-dark">
              <tr>
                <th>Department</th>
                <th>Declared</th>
                <th>{employeeTotalsByDepartment ? 'Total Employees' : 'Declared (Dept)'}</th>
                <th>%</th>
              </tr>
            </thead>
            <tbody>
            ...
            <tfoot>
              <tr className="fw-bold">
                ...
                <td>{employeeTotalsByDepartment ? Object.values(employeeTotalsByDepartment).reduce((a, v) => a + (typeof v === 'number' ? v : 0), 0) : rows.reduce((a, r) => a + r.total, 0)}</td>
                <td>100.0</td>
              </tr>
            </tfoot>
          </table>
