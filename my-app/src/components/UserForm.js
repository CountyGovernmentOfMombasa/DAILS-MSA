...
const UserFormInner = () => {
  ...
  const { declarationDate, periodStart, periodEnd, profile } = location.state || {};
  ...
  const isEditingExisting = !!editContext.declarationId;
    ...
    if (profile) {
      const employmentNature = profile.employment_nature || profile.nature_of_employment || '';
      return {
        ...profile,
  ...
  useEffect(() => {
    if (declarationType === 'biennial') {
      fetch('/api/settings/locks')
        ...
        .then(data => {
          if (data && data.locks) setBiennialLocked(Boolean(data.locks.biennial_declaration_locked));
        })
        .catch(() => setBiennialLocked(false));
    }
    if (model) {
      setUserData(prev => ({ ...prev, ...model.profile, declaration_type: model.type }));
      setIsLoading(false);
      return;
    }
    if (!profile) {
      ...
      fetch('/api/users/profile', { headers: { 'Authorization': `Bearer ${token}` } })
        .then(res => res.ok ? res.json() : null)
        .then(p => { if (p) setUserData(prev => ({ ...prev, ...p, declaration_type: declarationType })); })
        .catch(err => console.error('Error fetching user profile:', err))
        .finally(() => setIsLoading(false));
    } else {
      setIsLoading(false);
    }
  }, [declarationType, profile, token, navigate, location.state, location.search, model]);

  const { subToParent: SUB_DEPARTMENT_PARENT, subDepartments: SUB_DEPARTMENTS, loading: deptLoading, error: deptError, reload: reloadDepts } = useDepartments();
  ...
  const handleSubmit = (e) => {
    ...
    saveProgress({
      lastStep: 'spouse',
      stateSnapshot: { declarationDate, periodStart, periodEnd, userData: formattedUserData }
    }, key);
    scheduleServerSync(key, token);
    const nextPath = appendDeclarationIdToPath('/spouse-form', getEditContext({ locationState: location.state, locationSearch: location.search }).declarationId);
    navigate(nextPath, { state: {
      ...location.state,
      userData: { ...formattedUserData, declarationDate, periodStart, periodEnd },
      declarationDate,
      periodStart,
      periodEnd
    }});
            ...
            <div className="d-flex gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  const editPath = appendDeclarationIdToPath('/edit-declaration', ctx.declarationId);
            ...
            <Card className="shadow-lg border-0">
              <Card.Body className="p-5">
                <div className="text-center mb-4">
                  ...
                  <h2 className="fw-bold text-dark mb-2">Personal Information</h2>
                  <p className="text-muted">Step 1 of 4</p>
                  <ProgressBar now={25} className="mb-4" style={{ height: '8px' }} />
                </div>
                ...
                <Form onSubmit={handleSubmit}>
                      ...
                      <Row>
                        <Col md={4}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-surname" className="fw-semibold">i) Surname</Form.Label>
                            <Form.Control
                              id="userform-surname"
                        ...
                        <Col md={4}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-first-name" className="fw-semibold">ii) First Name</Form.Label>
                            <Form.Control
                              id="userform-first-name"
                              autoComplete="given-name"
                              type="text"
                              name="first_name"
                              value={existingData?.first_name || userData.first_name}
                              onChange={handleChange}
                              required
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                        ...
                        <Col md={4}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-other-names" className="fw-semibold">iii) Other Names</Form.Label>
                            <Form.Control
                              id="userform-other-names"
                              autoComplete="additional-name"
                              type="text"
                              name="other_names"
                              value={existingData?.other_names || userData.other_names}
                              onChange={handleChange}
                              required
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-birthdate" className="fw-semibold">i) Date of Birth</Form.Label>
                            <Form.Control
                              ...
                              value={toISODate(existingData?.birthdate || userData.birthdate)}
                        ...
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-place-of-birth" className="fw-semibold">ii) Place of Birth</Form.Label>
                            <Form.Control
                              id="userform-place-of-birth"
                              autoComplete="address-level2"
                              type="text"
                              name="place_of_birth"
                              value={existingData?.place_of_birth || userData.place_of_birth}
                              onChange={handleChange}
                              className="py-3"
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Form.Group className="mb-3">
                        <Form.Label htmlFor="userform-marital-status" className="fw-semibold">Marital Status</Form.Label>
                        <Form.Select
                          id="userform-marital-status"
                          autoComplete="marital-status"
                          name="marital_status"
                          value={existingData?.marital_status || userData.marital_status}
                          onChange={handleChange}
                          required
                          className="py-3"
                          style={{ borderRadius: '12px' }}
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-postal-address" className="fw-semibold">i) Postal Address</Form.Label>
                            <Form.Control
                              id="userform-postal-address"
                              autoComplete="postal-code"
                              type="text"
                              name="postal_address"
                              value={existingData?.postal_address || userData.postal_address}
                              onChange={handleChange}
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                        ...
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-physical-address" className="fw-semibold">ii) Physical Address</Form.Label>
                            <Form.Control
                              ...
                              value={existingData?.physical_address || userData.physical_address}
                              onChange={handleChange}
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                      ...
                      <Row>
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-email" className="fw-semibold">Email Address</Form.Label>
                            <Form.Control
                              id="userform-email"
                              autoComplete="email"
                              type="email"
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-national-id" className="fw-semibold">i) National ID Number</Form.Label>
                            <Form.Control
                              id="userform-national-id"
                              autoComplete="national-identification-number"
                              type="text"
                              name="national_id"
                              value={existingData?.national_id || userData.national_id}
                              onChange={handleChange}
                              required
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                        ...
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-payroll-number" className="fw-semibold">ii) Payroll Number (optional)</Form.Label>
                            <Form.Control
                              id="userform-payroll-number"
                              autoComplete="off"
                              type="text"
                              name="payroll_number"
                              value={existingData?.payroll_number || userData.payroll_number}
                              onChange={handleChange}
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                        ...
                        <Col md={6}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-sub-department" className="fw-semibold">iii) Sub Department</Form.Label>
                            <Form.Select
                              id="userform-sub-department"
                              aria-label="Sub Department"
                              name="sub_department"
                              value={existingData?.sub_department || userData.sub_department || ''}
                              onChange={handleChange}
                              className="py-3"
                              style={{ borderRadius: '12px' }}
                            >
                            ...
                            {deptLoading && <div className="small text-muted mt-1">Loading departments...</div>}
                            {deptError && <div className="small text-warning mt-1">Failed to load latest departments: {deptError} <button type="button" className="btn btn-sm btn-outline-secondary ms-2" onClick={reloadDepts}>Retry</button></div>}
                          </Form.Group>
                        </Col>
                        <Col md={4}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-department" className="fw-semibold">iii a) Department (auto)</Form.Label>
                            <Form.Control
                              id="userform-department"
                              type="text"
                              name="department"
                              value={existingData?.department || userData.department}
                              readOnly
                        ...
                        <Col md={4}>
                          <Form.Group className="mb-3">
                            <Form.Label htmlFor="userform-nature-employment" className="fw-semibold">iv) Nature of Employment</Form.Label>
                            <Form.Select
