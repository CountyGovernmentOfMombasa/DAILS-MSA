...
const DeclarationTypePage = () => {
  ...
  const handleSelect = (type) => {
    if ((type === 'biennial' && biennialLocked) || (type === 'first' && firstLocked) || (type === 'final' && finalLocked)) return;
  ...
  const handleModalProceed = () => {
    ...
    if (profile && profile.birthdate) {
      formattedProfile = { ...profile, birthdate: toISODate(profile.birthdate) };
    }
    // Persist period info to sessionStorage so downstream steps can recover
    try {
      sessionStorage.setItem('declarationPeriod', JSON.stringify({
  ...
  return (
    <div className="container py-5">
      ...
      <Modal show={showModal} onHide={handleModalClose} centered>
        ...
        <Modal.Body>
          <div className="mb-3">
            <label className="form-label fw-semibold">Date of Submission (DD/MM/YYYY)</label>
            <input
              type="text"
          ...
          <div className="mb-3">
            <label className="form-label fw-semibold">Period Start Date (DD/MM/YYYY)</label>
            <input
              type="text"
              className={`form-control py-3 ${validationErrors.length && !toISODate(periodStart) ? 'is-invalid' : ''}`}
              value={periodStart}
              onChange={e => { setPeriodStart(formatDate(e.target.value)); if (validationErrors.length) setValidationErrors([]); }}
          ...
          <div className="mb-3">
            <label className="form-label fw-semibold">Period End Date (DD/MM/YYYY)</label>
            <input
              type="text"
              className={`form-control py-3 ${validationErrors.length && !toISODate(periodEnd) ? 'is-invalid' : ''}`}
              value={periodEnd}
              onChange={e => { setPeriodEnd(formatDate(e.target.value)); if (validationErrors.length) setValidationErrors([]); }}
              placeholder="DD/MM/YYYY"
              pattern="\d{2}/\d{2}/\d{4}"
              style={{ borderRadius: '12px' }}
              required
          ...
          {validationErrors.length > 0 && (
            <div className="mt-3">
              {validationErrors.map((err,i)=>(<div key={i} className="alert alert-danger py-2 mb-2" style={{borderRadius:'8px'}}>{err}</div>))}
            </div>
          )}
          </div>
          <span className="text-muted">These dates will be used as the official period of your declaration.</span>
          ...
          {pendingType === 'biennial' && (() => {
            ...
            return (
              <Alert variant="info" className="mt-3">
                <strong>Biennial Declaration Guidance:</strong><br />
                The period end date is fixed to <b>31/10/{year}</b>.<br />
                If you joined the County after <b>01/11/{prevDeclarationYear}</b>, use your <b>date of appointment</b> as the start date. Otherwise use <b>01/11/{prevDeclarationYear}</b> as the start date.
              </Alert>
            );
          })()}
          {pendingType === 'first' && (
            <Alert variant="info" className="mt-3">
              <strong>First (Initial) Declaration Guidance:</strong><br />
              Your declaration period should cover <b>one year prior to your date of appointment up to your date of appointment</b>. Set the end date to your appointment date and the start date to one year earlier.
            </Alert>
          )}
          {pendingType === 'final' && (() => {
            const parseDDMMYYYY = (s) => {
            ...
            const previousDeclarationYear = year % 2 === 0 ? year - 1 : year - 2; // last odd year cycle
            return (
              <Alert variant="info" className="mt-3">
                <strong>Final Declaration Guidance:</strong><br />
                Set the start date to <b>31/10/{previousDeclarationYear}</b> (the end date of your last declaration). The end date should be your <b>last day of service</b> with the County.
              </Alert>
            );
          })()}
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleModalProceed} disabled={!periodStart || !periodEnd}>
            Proceed
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <div className="row">
        <div className="col-md-4 mb-3">
          <button
            ...
            title={firstLocked ? 'First Declaration is currently locked by the administrator.' : ''}
          >
            Initial Declaration
            {firstLocked && (
              <span style={{ color: 'red', fontSize: '0.9em', display: 'block' }}>
                (Locked)
              </span>
            )}
          </button>
        ...
        <div>
          {firstLocked && (
            <Alert variant="danger" className="mb-3">
              ...
              <strong>First Declaration is currently locked by the administrator. You cannot submit a first declaration at this time.</strong>
            </Alert>
          )}
          <Alert variant="info" className="mb-3">
            <i className="fas fa-info-circle me-2"></i>
            <strong>Note:</strong> This is for new workers who have just joined the county and should be done within the first 30 days of joining.
          </Alert>
        </div>
        <div className="col-md-4 mb-3">
          <button
            ...
            title={biennialLocked ? 'Biennial Declaration is currently locked by the administrator.' : ''}
          >
          ...
          <div>
            <Alert variant="info" className="mb-3">
              <i className="fas fa-info-circle me-2"></i>
              <strong>Note:</strong> This is the biennial declaration that must be completed before 31st of December.
              Biennial declarations can <u>only</u> be submitted between <b>November 1</b> and <b>December 31</b> of an <b>odd year</b> (e.g., 2025, 2027, etc.).
            </Alert>
          </div>
        <div className="col-md-4 mb-3">
          <button
            ...
            title={finalLocked ? 'Final Declaration is currently locked by the administrator.' : ''}
          >
            Final Declaration
            {finalLocked && (
              <span style={{ color: 'red', fontSize: '0.9em', display: 'block' }}>
        ...
        <div>
          {finalLocked && (
            <Alert variant="danger" className="mb-3">
              <i className="fas fa-lock me-2"></i>
              <strong>Final Declaration is currently locked by the administrator. You cannot submit a final declaration at this time.</strong>
            </Alert>
          )}
          <Alert variant="info" className="mb-3">
            <i className="fas fa-info-circle me-2"></i>
            <strong>Note:</strong> This is only for the workers who have resigned or retired from the County.
          </Alert>
        </div>
      </div>
    </div>
