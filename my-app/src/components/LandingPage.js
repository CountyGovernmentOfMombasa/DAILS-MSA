...
const LandingPage = () => {
  ...
  const handleExportPDF = async (declaration) => {
    try {
      ...
      const res = await fetch(`/api/declarations/${declaration.id}/download-pdf`, {
      ...
      if (instructionHeader) {
        ...
        setPdfToast({ show: true, message: 'The password for the attached PDF is Your National ID number.' });
      }
  ...
  useEffect(() => {
    if (!profile) return;
    const normalizedBirthdate = (profile.birthdate && profile.birthdate !== '0000-00-00') ? profile.birthdate : '';
    setForm({
      ...profile,
  ...
  const handleChange = (e) => {
    ...
    if (name === 'sub_department') {
      ...
      setForm(prev => ({ ...prev, sub_department: value, department: parent }));
      return;
    }
  ...
  // Merge new form values into context profile if available
        setProfile((prev) => ({ ...(prev || {}), ...form, birthdate: form.birthdate && form.birthdate !== '0000-00-00' ? form.birthdate : '' }));
    } catch (e) {
  ...
  const handleViewDeclaration = async (declaration) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/declarations/${declaration.id}`, { headers: { 'Authorization': `Bearer ${token}` }});
  ...
  const renderFinSection = (label, arr) => {
    const filtered = (arr || []).filter(r => (r.description || '').trim() || r.value);
    if (!filtered.length) return <div className="mb-1"><strong>{label}:</strong> <span className="text-muted">None</span></div>;
    return (
      <div className="mb-1">
        <strong>{label}:</strong>
          ...
          {filtered.map((r,i) => (
            <li key={i}>
              ...
              {r.value !== undefined && r.value !== '' && (
                <span className={label === 'Liabilities' ? 'text-danger ms-1' : label === 'Assets' ? 'text-primary ms-1' : 'text-success ms-1'}>
  ...
  const renderPrevTable = () => {
    ...
    const pageSlice = (Array.isArray(declarations) ? declarations : []).slice(startIdx, startIdx + declPageSize);
    const from = total === 0 ? 0 : startIdx + 1;
    const to = startIdx + pageSlice.length;
    return (
      <>
        <div className="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
          <div className="small text-muted">Showing {from}-{to} of {total}</div>
          <div className="d-flex align-items-center gap-2">
            <Form.Select size="sm" style={{ width: 'auto' }} value={declPageSize} onChange={(e)=>{ setDeclPageSize(parseInt(e.target.value,10)); setDeclPage(1); }} aria-label="Select page size">
              {[5,10,20,50].map(sz => <option key={sz} value={sz}>{sz}/page</option>)}
            </Form.Select>
            <div className="btn-group" role="group" aria-label="Pagination controls">
              <Button size="sm" variant="outline-secondary" disabled={currentPage === 1} onClick={()=>setDeclPage(p=>Math.max(1,p-1))}>&laquo;</Button>
              <Button size="sm" variant="outline-secondary" disabled>{currentPage} / {totalPages}</Button>
              <Button size="sm" variant="outline-secondary" disabled={currentPage === totalPages} onClick={()=>setDeclPage(p=>Math.min(totalPages,p+1))}>&raquo;</Button>
            </div>
          </div>
        </div>
        <Table striped bordered hover responsive>
          ...
          <tbody>
            {pageSlice.map((decl, localIdx) => {
              ...
              return (
                <tr key={decl.id}>
                  ...
                  <td>
                    <Badge bg={decl.status === 'approved' ? 'success' : decl.status === 'pending' ? 'warning' : decl.status === 'rejected' ? 'danger' : 'secondary'}>
                      {formatStatus(decl.status)}
                    </Badge>
                  </td>
                  <td>
                    <Button size="sm" variant="primary" onClick={() => handleViewDeclaration(decl)}>View</Button>{' '}
                    <Button size="sm" variant="secondary" onClick={() => handleExportPDF(decl)}>Export to PDF</Button>{' '}
                    {(decl.status === 'pending' || decl.status === 'rejected' || (decl.status === 'approved' && (decl.user_edit_count||0) < 1)) && (
                      <Button size="sm" variant="warning" onClick={() => handleEditDeclaration(decl)} disabled={decl.user_edit_count >= 1 && decl.status === 'approved'}>Edit</Button>
                    )}
                  </td>
                </tr>
              );
            ...
            {!pageSlice.length && (
              <tr><td colSpan={6} className="text-center text-muted">No declarations on this page.</td></tr>
            )}
          </tbody>
        </Table>
      </>
        ...
        {hasAdminAccess && (
          adminToken ? (
            <Button
              ...
              disabled={elevating}
            >
              Admin Dashboard {roleAbbrev && <span className="badge bg-primary ms-2" data-testid="role-badge">{roleAbbrev}</span>}
            </Button>
          ) : (
            <Button
      ...
      <Container className="py-5">
        <div className="text-center mb-5">
          ...
          <h1 className="display-4 fw-bold text-dark mb-4" 
              style={{ background: 'linear-gradient(45deg, var(--primary-blue), var(--secondary-green))', 
                       WebkitBackgroundClip: 'text', 
                       WebkitTextFillColor: 'transparent' }}>
            County Government of Mombasa Employee Portal
          </h1>
          <p className="lead text-muted">
            Please select the form you'd like to complete or access external services
          </p>
          {elevationError && (
  ...
  {/* Profile Section */}
        <Card className="mb-5 shadow border-0">
          <Card.Body>
            ...
            <Alert variant="warning" className="mb-4">
              Please review your profile and update any information that is not accurate.
              Click <strong>Edit Profile</strong> to make changes, then select <strong>Save</strong>.
            </Alert>
              ...
              <>
                <Form onSubmit={handleSave}>
                  ...
                  <Row>
                    ...
                    <Col md={4}><Form.Group className="mb-3"><Form.Label htmlFor="profile-place-of-birth">Place of Birth</Form.Label><Form.Control id="profile-place-of-birth" name="place_of_birth" value={form.place_of_birth || ''} onChange={handleChange} disabled={!editMode} /></Form.Group></Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        <Form.Label htmlFor="profile-marital-status">Marital Status</Form.Label>
                        <Form.Select
                  ...
                  <Row>
                    ...
                    <Col md={3}><Form.Group className="mb-3"><Form.Label htmlFor="profile-payroll-number">Payroll Number</Form.Label><Form.Control id="profile-payroll-number" name="payroll_number" value={form.payroll_number || ''} onChange={handleChange} disabled={!editMode} /></Form.Group></Col>
                  </Row>
                  <Row>
                    <Col md={4}><Form.Group className="mb-3"><Form.Label htmlFor="profile-designation">Designation</Form.Label><Form.Control id="profile-designation" name="designation" value={form.designation || ''} onChange={handleChange} disabled={!editMode} /></Form.Group></Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        <Form.Label htmlFor="profile-sub-department">Sub Department</Form.Label>
                        <Form.Select
                        ...
                        {editMode && !form.sub_department && (
                          <div className="form-text text-danger">Sub department is required.</div>
                        )}
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        <Form.Label htmlFor="profile-department">Department (auto)</Form.Label>
                        <Form.Control
                  ...
                  <Row>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        <Form.Label htmlFor="profile-nature-of-employment">Nature of Employment</Form.Label>
                        <Form.Select
                    ...
                    <div className="d-flex gap-2">
                      ...
                      <Button type="button" variant="secondary" onClick={() => {
                        ...
                        setForm({
                          ...
                          nature_of_employment: profile && profile.nature_of_employment ? profile.nature_of_employment : '',
                          sub_department: profile && profile.sub_department ? profile.sub_department : ''
                        });
                      }}>Cancel</Button>
                    </div>
                  )}
        ...
        <Card className="mb-5 shadow border-0">
          <Card.Body>
            <h3 className="fw-bold mb-4 text-primary">My Previous Declarations</h3>
            {declarationsLoading ? (
              <Spinner animation="border" />
            ) : (declarationsError) ? (
              <>
                {declarationsError && <Alert variant="danger" className="d-flex justify-content-between align-items-center">{declarationsError}<Button size="sm" variant="outline-light" onClick={handleRetryDeclarations}>Retry</Button></Alert>}
            ...
            {progress && (
              <div className="mt-3">
                <Alert variant="info" className="d-flex justify-content-between align-items-center">
                  <div>
                    You have an in-progress declaration last updated {timeAgo(progress.updatedAt)} at step: <strong>{progress.lastStep}</strong>
                    <div className="small mt-2">
                      {(() => {
                        ...
                        return <span>Progress: {pct}% ({completed}/{totalSteps} steps)</span>;
                      })()}
                    </div>
                  </div>
                  <div className="d-flex gap-2">
                    <Button size="sm" variant="primary" onClick={handleResumeProgress}>Resume</Button>
                    <Button size="sm" variant="outline-danger" onClick={() => setShowDiscardModal(true)}>Discard</Button>
                    <Button size="sm" variant="outline-secondary" onClick={handleRestoreFromServer}>Restore</Button>
                  </div>
                </Alert>
              </div>
            )}
        ...
        <Modal show={showDiscardModal} onHide={() => setShowDiscardModal(false)} centered>
          ...
          <Modal.Body>
            This will permanently remove your locally saved progress for the current declaration. Submitted declarations are unaffected. Do you want to continue?
          </Modal.Body>
            <Modal.Footer>
              <Button variant="secondary" onClick={() => setShowDiscardModal(false)}>Cancel</Button>
              <Button variant="danger" onClick={handleDiscardProgress}>Discard</Button>
            </Modal.Footer>
        </Modal>
        <ToastContainer position="bottom-end" className="p-3">
          <Toast bg="info" onClose={() => setResumeToast({ show: false, step: '' })} show={resumeToast.show} delay={3000} autohide>
            <Toast.Header closeButton={true}>
              <strong className="me-auto">Resume</strong>
              <small>Now</small>
            </Toast.Header>
            <Toast.Body className="text-white">Resumed declaration at step: <strong>{resumeToast.step}</strong></Toast.Body>
          </Toast>
          <Toast bg="dark" onClose={() => setPdfToast({ show: false, message: '' })} show={pdfToast.show} delay={5000} autohide>
            <Toast.Header closeButton={true}>
        ...
        <Row className="justify-content-center">
          <Col lg={3} md={6} className="mb-4">
            <div className="text-decoration-none" style={{ cursor: 'pointer' }}
              onClick={() => navigate('/select-declaration-type', { state: { profile } })}>
              <Card className="h-100 shadow-sm border-primary border-2 hover-card" 
          ...
          <Col lg={3} md={6} className="mb-4">
            <a href="https://survey.mcpsb.go.ke/" 
               target="_blank" 
               rel="noopener noreferrer" 
               className="text-decoration-none">
              <Card className="h-100 shadow-sm border-info border-2 hover-card" 
                    style={{ transition: 'all 0.3s ease', cursor: 'pointer' }}>
                <Card.Body className="text-center p-4">
                  <div className="mb-4">
                    <div className="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3"
                      ...
                      <i className="fas fa-poll text-white" style={{ fontSize: '2rem' }}></i>
                    </div>
                  </div>
          ...
          <Col lg={3} md={6} className="mb-4">
            <a href="https://ictsupport.mombasa.go.ke/" 
               target="_blank" 
               rel="noopener noreferrer" 
               className="text-decoration-none">
              <Card className="h-100 shadow-sm border-warning border-2 hover-card" 
                    style={{ transition: 'all 0.3s ease', cursor: 'pointer' }}>
                <Card.Body className="text-center p-4">
                  <div className="mb-4">
                    <div className="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3"
                      ...
                      <i className="fas fa-headset text-white" style={{ fontSize: '2rem' }}></i>
                    </div>
                  </div>
          ...
          <Col lg={3} md={6} className="mb-4">
            <div className="text-decoration-none" style={{ cursor: 'pointer' }} onClick={handleAdminAccess}>
              <Card className="h-100 shadow-sm border-success border-2 hover-card" 
                    style={{ transition: 'all 0.3s ease', cursor: 'pointer' }}>
                <Card.Body className="text-center p-4">
                  <div className="mb-4">
                    <div className="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3"
        ...
        <Modal size="xl" show={showDeclModal} onHide={() => { setShowDeclModal(false); setSelectedDecl(null); }} backdrop="static" centered scrollable>
          <Modal.Header closeButton>
          ...
          <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            {declModalError && !declModalLoading && (
              <Alert variant="danger" className="mb-0">{declModalError}</Alert>
            )}
            {selectedDecl && !declModalLoading && !declModalError && (
              <div>
                ...
                <Row className="small mb-4 g-3">
                  <Col md={3}><strong>Name:</strong> {(selectedDecl.first_name||'') + ' ' + (selectedDecl.other_names||'') + ' ' + (selectedDecl.surname||'')}</Col>
                  <Col md={3}><strong>Marital Status:</strong> {selectedDecl.marital_status || '—'}</Col>
                  <Col md={3}><strong>Email:</strong> {selectedDecl.email || '—'}</Col>
                  <Col md={3}><strong>Payroll #:</strong> {selectedDecl.payroll_number || '—'}</Col>
                  <Col md={3}><strong>Department:</strong> {selectedDecl.department || '—'}</Col>
                  <Col md={3}><strong>Birthdate:</strong> {selectedDecl.birthdate || '—'}</Col>
                  <Col md={3}><strong>Place of Birth:</strong> {selectedDecl.place_of_birth || '—'}</Col>
                  <Col md={3}><strong>Nature of Employment:</strong> {selectedDecl.nature_of_employment || '—'}</Col>
                </Row>
                <h5 className="mb-2">Spouses ({(selectedDecl.spouses||[]).length})</h5>
                {(!selectedDecl.spouses || !selectedDecl.spouses.length) ? <p className="text-muted small">None</p> : (
                  <Table size="sm" bordered responsive className="mb-4">
                    <thead><tr className="small"><th>Name</th><th>Income Items</th><th>Assets</th><th>Liabilities</th></tr></thead>
                    <tbody>
                      {selectedDecl.spouses.map(s => {
                        ...
                        return (
                          <tr key={s.id} className="small">
                            <td>{s.full_name || [s.first_name, s.other_names, s.surname].filter(Boolean).join(' ')}</td>
                            <td>{income.filter(r=>r.description||r.value).length}</td>
                            <td>{assets.filter(r=>r.description||r.value).length}</td>
                            <td>{liabilities.filter(r=>r.description||r.value).length}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <h5 className="mb-2">Children ({(selectedDecl.children||[]).length})</h5>
                {(!selectedDecl.children || !selectedDecl.children.length) ? <p className="text-muted small">None</p> : (
                  <Table size="sm" bordered responsive className="mb-4">
                    <thead><tr className="small"><th>Name</th><th>Income Items</th><th>Assets</th><th>Liabilities</th></tr></thead>
                    <tbody>
                      {selectedDecl.children.map(c => {
                        ...
                        return (
                          <tr key={c.id} className="small">
                            <td>{c.full_name || [c.first_name, c.other_names, c.surname].filter(Boolean).join(' ')}</td>
                            <td>{income.filter(r=>r.description||r.value).length}</td>
                            <td>{assets.filter(r=>r.description||r.value).length}</td>
                            <td>{liabilities.filter(r=>r.description||r.value).length}</td>
                          </tr>
                ...
                {(!selectedDecl.financial_unified || !selectedDecl.financial_unified.length) ? <p className="text-muted small">No financial entries</p> : (
                    ...
                    return (
                      <Card key={idx} className="mb-3 border-0 shadow-sm">
                        <Card.Header className="py-2 d-flex justify-content-between align-items-center">
                          <div className="small"><strong>{m.member_name}</strong> <span className="text-muted">({m.member_type})</span></div>
                          <div className="small">
                            <Badge bg="secondary" className="me-1">Income {income.filter(r=>r.description||r.value).length}</Badge>
                            <Badge bg="primary" className="me-1">Assets {assets.filter(r=>r.description||r.value).length}</Badge>
                            <Badge bg="danger">Liabilities {liabilities.filter(r=>r.description||r.value).length}</Badge>
                          </div>
                        </Card.Header>
                        <Card.Body className="pt-2 small">
                          ...
                          {m.data?.other_financial_info && <div className="mt-2"><strong>Notes:</strong> {m.data.other_financial_info}</div>}
                        </Card.Body>
                      </Card>
                    );
                  })
                ...
                {selectedDecl.witness_name || selectedDecl.witness_phone ? (
                  <div className="small mb-2">
                    <div><strong>Name:</strong> {selectedDecl.witness_name || '—'}</div>
                    <div><strong>Phone:</strong> {selectedDecl.witness_phone || '—'}</div>
                    <div><strong>Address:</strong> {selectedDecl.witness_address || '—'}</div>
                    <div><strong>Signed:</strong> {selectedDecl.witness_signed ? 'Yes' : 'No'}</div>
                  </div>
                ) : <p className="text-muted small">No witness info</p>}
                {selectedDecl.correction_message && (
                  <Alert variant="info" className="small py-2">Correction: {selectedDecl.correction_message}</Alert>
                )}
              </div>
            )}
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={() => { setShowDeclModal(false); setSelectedDecl(null); }}>Close</Button>
            {selectedDecl && (
              <Button variant="outline-secondary" onClick={() => handleExportPDF(selectedDecl)}>Export PDF</Button>
            )}
          </Modal.Footer>
        </Modal>
      </Container>
