        ...
        <Table striped bordered hover responsive>
          ...
          <tbody>
            {pageSlice.map((decl, localIdx) => {
              ...
              const periodStart =
                (firstFinDecl && firstFinDecl.period_start_date) ||
                decl.period_start_date;
              const periodEnd =
                (firstFinDecl && firstFinDecl.period_end_date) ||
              ...
              return (
                <tr key={decl.id}>
                  ...
                  <td>{declDate ? formatDate(declDate) : "N/A"}</td>
                  <td>
                    {periodStart && periodEnd
                      ? `${formatDate(periodStart)} to ${formatDate(periodEnd)}`
                      : "N/A"}
          ...
          <Card className="mb-5 shadow border-0">
            <Card.Body>
              ...
              <Alert variant="warning" className="mb-4">
                Please review your profile and update any information that is
                not accurate. Click <strong>Edit Profile</strong> to make
                changes, then select <strong>Save</strong>.
              </Alert>
              {loading ? (
              ...
              {declarationsLoading ? (
                ...
                <>
                  ...
                  {declarations.length === 0 ? (
                    <Alert variant="info">
                      No previous declarations found.
                    </Alert>
                  ) : (
                    renderPrevTable()
                  )}
                  ...
                  <Row className="mb-4 small g-3">
                    ...
                    <Col md={3}>
                      ...
                      {selectedDecl.declaration_date
                        ? formatDate(selectedDecl.declaration_date)
                        : "—"}
                    </Col>
                    <Col md={3}>
                      <strong>Period Start:</strong>{" "}
                      {selectedDecl.period_start_date
                        ? formatDate(selectedDecl.period_start_date)
                        : "—"}
                    </Col>
                    <Col md={3}>
                      <strong>Period End:</strong>{" "}
                      {selectedDecl.period_end_date
                        ? formatDate(selectedDecl.period_end_date)
                        : "—"}
                    </Col>
                    <Col md={3}>
