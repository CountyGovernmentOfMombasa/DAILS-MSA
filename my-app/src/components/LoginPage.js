...
const LoginPage = () => {
    ...
    useEffect(() => {
        ...
        if (stepActive && storedToken) {
            ...
            const until = parseInt(sessionStorage.getItem('otpCooldownUntil') || '0', 10);
            ...
            const response = await fetch('/api/auth/login', {
             ...
             if (response.ok) {
                ...
                if (response.status === 423) {
                    // Account locked
                    setError(errorData.message || 'Account suspended due to multiple failed attempts.');
                    return;
                }
                if (response.status === 429) {
                    setError(errorData.message || 'Too many attempts. Please wait and try again.');
                    return;
                }
    ...
    const handleResendOtp = async () => {
        ...
        try {
            ...
            await resendOtp({ nationalId: nationalId.trim(), password: 'Change@001' });
            ...
            if (e?.response?.status === 429) {
                setError('You’ve requested codes too often. Please wait and try again.');
            } else {
                            ...
                            <Card.Body className="p-5">
                                ...
                                <Form onSubmit={handleLogin}>
                                    <Form.Group className="mb-3">
                                        <Form.Label className="fw-semibold">National ID Number</Form.Label>
                                        <Form.Control
                                            type="text"
                                    ...
                                    {requirePhone && (
                                        <Form.Group className="mb-3">
                                            ...
                                            <Form.Control
                                                ...
                                                value={phoneNumber}
                                                onChange={(e) => { const v = e.target.value; setPhoneNumber(v); const n = normalizePhone(v); sessionStorage.setItem('loginPhoneNormalized', n); 
                                            ...
                                            <Alert variant="info" className="mt-2">
                                                Please provide your phone number to continue. We'll normalize it (e.g., 0712345678).
                                            </Alert>
                                        </Form.Group>
                                    )}
                                    <Form.Group className="mb-3">
                                        ...
                                        {showDefaultPasswordInfo && (
                                            <Alert variant="info" className="mt-2">
                                                Your default password is <b>Change@001</b>. Please change it after logging in.
                                            </Alert>
                                        )}
                                    </Form.Group>
                                    ...
                                    <div className="d-flex justify-content-between align-items-center mb-3 mt-2">
                                        <button type="button" className="btn btn-link p-0 small" onClick={() => setShowForgot(true)}>Forgot password?</button>
                                ...
                                {showForgot && !otpRequired && (
                                    <div>
                                        <div className="mb-2">
                                            <button type="button" className="btn btn-link p-0 small" onClick={() => setShowForgot(false)}>&larr; Back to login</button>
                                        </div>
                                        <ForgotPasswordUser />
                                    </div>
                                )}
                                ...
                                {otpRequired && !showForgot && (
                                    <Form onSubmit={handleVerifyOtp}>
                                        ...
                                        {error && (
                                            <Alert variant={/expired/i.test(error) ? 'warning' : 'danger'} className="mb-3">{error}</Alert>
                                        )}
                                        {info && (
                                            <Alert variant="info" className="mb-3">{info}</Alert>
                                        )}
                                        ...
                                        <div className="d-flex gap-2">
                                            <Button
                                                ...
                                                disabled={isLoading}
                                                className="w-100 py-3 fw-semibold"
                                                style={{ borderRadius: '12px', background: 'linear-gradient(45deg, var(--primary-blue), #0056b3)', border: 'none' }}
                                            >
