...
import { formatToAppTimezone } from "../util/date";
...
function StatusAuditModule({ adminToken, isSuper }) {
    ...
    async (targetPage = 1) => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/admin/declarations/status-audit/global?${qs.toString()}`,
  ...
  useEffect(() => {
    if (isSuper) fetchAudit(1);
  }, [fetchAudit, isSuper]);

  const resetFilters = () => {
    setFilters({ status: "", admin: "", national_id: "", from: "", to: "" });
    setPage(1);
  };
  ...
  const exportExcel = () => {
    ...
    const data = rows.map((r) => ({
      ...
      PreviousStatus:
        r.previous_status === "pending" ? "Submitted" : r.previous_status || "",
      ...
      ChangedAt: formatToAppTimezone(r.changed_at),
    }));
  ...
  const openDetail = async (declarationId) => {
    ...
    try {
      const declRes = await fetch(`/api/admin/declarations/${declarationId}`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
      if (declRes.ok) {
    ...
    try {
      const audRes = await fetch(
        `/api/admin/declarations/${declarationId}/status-audit?limit=100`,
        { headers: { Authorization: `Bearer ${adminToken}` } }
      );
      if (audRes.ok) {
  ...
  const setQueryParam = (id) => {
    ...
    if (id) url.searchParams.set("declarationId", id);
    else url.searchParams.delete("declarationId");
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-header d-flex justify-content-between align-items-center">
        <h5 className="mb-0">
          <i className="bi bi-layers me-2"></i>Status Audit
        </h5>
        ...
        <div className="row g-2 mb-3">
          ...
          <div className="col-auto">
            ...
            <select
              ...
              value={limit}
              onChange={(e) => {
                setLimit(parseInt(e.target.value) || PAGE_LIMIT_DEFAULT);
                setPage(1);
              }}
            >
              {[25, 50, 100, 200].map((n) => (
              ...
              {loading ? (
                ...
                rows.map((r) => (
                  <tr
                    ...
                    onClick={() => openDetailWithLink(r.declaration_id)}
                  ...
                  >
                    ...
                    <td>
                      <span className="badge bg-secondary text-capitalize me-1">
                        {r.previous_status || "∅"}
                      </span>
                      →
                      <span className="badge bg-primary text-capitalize ms-1">
                        {r.status === "pending" ? "Submitted" : r.status}
                      </span>
                    </td>
                    <td className="text-truncate" style={{ maxWidth: "220px" }}>
                      {r.new_correction_message || ""}
                    </td>
                    <td>{r.admin_username || "—"}</td>
                    <td>{formatToAppTimezone(r.changed_at)}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="d-flex align-items-center justify-content-between mt-2">
          <div className="btn-group btn-group-sm">
            ...
            <button
              ...
              onClick={() => {
                if (page < pages) fetchAudit(page + 1);
              }}
            >
              Next
            </button>
              ...
              >
                ...
                {detailDeclaration && (
                  <div className="mb-3">
                    ...
                    <div className="small">
                      <div>
                        <strong>ID:</strong> {detailDeclaration.id}
                      </div>
                      <div>
                        <strong>User:</strong>{" "}
                        {[
                          detailDeclaration.first_name,
                          detailDeclaration.other_names,
                          detailDeclaration.surname,
                        ]
                          .filter(Boolean)
                          .join(" ")}
                      </div>
                      <div>
                        ...
                        {detailDeclaration.payroll_number}
                      </div>
                      <div>
                        <strong>Department:</strong>{" "}
                        {detailDeclaration.department || "—"}
                      </div>
                      <div>
                        <strong>Type:</strong>{" "}
                        {detailDeclaration.declaration_type || "—"}
                      </div>
                      <div>
                        <strong>Status:</strong>{" "}
                        {detailDeclaration.status === "pending"
                          ? "Submitted"
                          : detailDeclaration.status === "rejected"
                          ? "Requesting Clarification"
                          : detailDeclaration.status}
                      </div>
                      <div>
                        <strong>Submitted:</strong>{" "}
                        {formatToAppTimezone(
                          detailDeclaration.submitted_at ||
                            detailDeclaration.declaration_date
                        )}
                      </div>
                    </div>
                  </div>
                ...
                {!detailAuditLoading && detailAudit.length === 0 && (
                  <div className="text-muted small">No audit records.</div>
                )}
                {!detailAuditLoading && detailAudit.length > 0 && (
                      ...
                      <tbody>
                        {detailAudit.map((a) => (
                          <tr key={a.id}>
                            ...
                            >
                              {a.previous_correction_message || ""}
                            </td>
                            <td
                              className="text-truncate"
                              style={{ maxWidth: "140px" }}
                            >
                              {a.new_correction_message || ""}
                            </td>
                            <td>{formatToAppTimezone(a.changed_at)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
